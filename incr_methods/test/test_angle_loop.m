...
successively added till their cumulative area does not exceed pi.*(0.5.*D).^2
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for j_1 = 1:size_points_path-1
            if isempty(all_points_path{i,j_1}) || isempty(all_points_path{i,j_1+1})
                break
            else
            cos_angle = sum(cell2mat(all_points_path{i,j_1}(4:6)).*cell2mat(all_points_path{i,j_1+1}(4:6)))/(norm(cell2mat(all_points_path{i,j_1}(4:6)))*norm(cell2mat(all_points_path{i,j_1+1}(4:6))));
            cos_angles(j_1) = {cos_angle};
            end
        end
        %%%
        cos_angles_1 = cell2mat(cos_angles);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for j_1 = 1:size_points_path-1
            if cos_angles_1(j_1) < cos(pi/18)
                x_ori = x_oris(j_1) + x_incr/2;
                index_surface_section = zeros(size_tri,1).*NaN;
                n = 1;
                for i_1 = 1:size_tri
                    if triangles(i_1,14)>=x_ori && triangles(i_1,16)+2>=y_ori && triangles(i_1,13)<=x_ori+D && triangles(i_1,15)<=y_ori+D+2
                ...
                switch isempty(index_surface_section)
                    case 0
                        ...
                        % 据曲面上的点沿法线方向偏移一个offset，法线反向
                        [point_surface_section,normal_surface_section] = offsetting_1(h,point_surface_section,normal_surface_section);
                        points_path(k+1,:) = [point_surface_section,normal_surface_section];
                        cell_points_path = num2cell(points_path(k+1,:));
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        A = all_points_path(i,1:j_1);
                        B = all_points_path(i,j_1+1:size_points_path);
                        size_points_path_1 = size(A,2);
                        A{size_points_path_1+1} = cell_points_path;
                        j_3 = 1;
                        for j_2 = size_points_path_1+2:size_points_path_1+1+size(B,2)
                        A{j_2} = B{j_3};
                        j_3 = j_3+1;
                        end
                        all_points_path{i,size_points_path+1} = 0;
                        all_points_path(i,:) = A;
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        if size(all_points_path,1) ~= i
                            break
