...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
size_pass = size(all_points_path,1);
size_points_path = size(all_points_path,2);
for i_3 = 1:size_pass
for j_1 = 1:size_points_path
    points_mat(j_1,:) = cell2mat(all_points_path{i_3,j_1});
end
for j_2 = 1:6
    points_moving_average(:,j_2) = movmean(points_mat(:,j_2),6);
end
all_points_moving_average(i_3,:) =  {points_moving_average};
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
m = zeros(1,size_points_path-1);
for i_4 = 1:size_pass
for j_3 = 1:size(all_points_moving_average{i_4,1},1)-1
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if isempty(all_points_path{i_4,j_3+1}) || isempty(all_points_path{i_4,j_3})
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        j_3 = j_3+1;
        %%%%%%%%
    else
        m(j_3) = norm(all_points_moving_average{i_4,1}(j_3+1,1:3)-all_points_moving_average{i_4,1}(j_3,1:3));
    end
end
cell_m = num2cell(m);
M(i_4) = {cell_m};
...
for i_4 = 1:size_pass
scatter3(all_points_moving_average{i_4,1}(:,1),all_points_moving_average{i_4,1}(:,2),all_points_moving_average{i_4,1}(:,3),10,'green','filled')
hold on
quiver3(all_points_moving_average{i_4,1}(:,1),all_points_moving_average{i_4,1}(:,2),all_points_moving_average{i_4,1}(:,3),all_points_moving_average{i_4,1}(:,4),all_points_moving_average{i_4,1}(:,5),all_points_moving_average{i_4,1}(:,6),'c')
hold on
end
axis equal
quiver3(points_path(:,1),points_path(:,2),points_path(:,3),points_path(:,4),points_path(:,5),points_path(:,6),'m')
hold on
scatter3(points_path(:,1),points_path(:,2),points_path(:,3),10,'red','filled')
